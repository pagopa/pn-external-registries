...
public class DeliveryClient extends CommonBaseClient {
    ...
    public Mono<SentNotificationV25> getSentNotificationPrivate(String iun) {
        ...
        return pnDeliveryApi.getSentNotificationPrivate(iun)
                .doOnError(throwable ->
                    log.logInvokationResultDownstreamFailed(PN_DELIVERY, String.format("Sent notification not found for iun: %s. Exception: %s - %s", iun, throwable.getClass().getSimpleName(), throwable.getMessage()),throwable)
                )
